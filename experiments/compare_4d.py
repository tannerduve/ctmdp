"""
4D gridworld experiment: Box Product vs Cartesian Product.
Shows action space advantage: BP has 8 actions, CP has 16 actions (2x).
"""
...
def optimal_policy_cp(cp):
    """Optimal: always choose 'next-next-next-next'."""
    desc = {}
...
def main():
    print("="*70)
    print("4D Gridworld Experiment: BP vs CP")
    print("="*70)
    
    size = 3
    num_runs = 10
    
    print(f"\n{size}×{size}×{size}×{size} grid:")
    ...
    bp_temp1 = box_product(path1, path2)
    bp_temp2 = box_product(bp_temp1, path3)
    bp = box_product(bp_temp2, path4)
    
    # Cartesian product: compose sequentially
    cp_temp1 = cartesian_product(path1, path2)
    cp_temp2 = cartesian_product(cp_temp1, path3)
    cp = cartesian_product(cp_temp2, path4)
    ...
    interior = (((size // 2, size // 2), size // 2), size // 2)
    ...
    print(f"  BP actions/state (interior): {bp_actions} (2+2+2+2=8)")
    print(f"  CP actions/state (interior): {cp_actions} (2×2×2×2=16)")
    if bp_actions > 0:
    ...
    # Run trials
    print(f"\nRunning {num_runs} trials (2000 episodes each)...")
    bp_dists = []
    cp_dists = []
    
    for run in range(num_runs):
        bp_dists.append(run_trial(bp, opt_bp, episodes=2000, max_steps=size*5))
        cp_dists.append(run_trial(cp, opt_cp, episodes=2000, max_steps=size*5))
        if (run + 1) % 5 == 0:
    ...
    print(f"\n{'='*70}")
    print("In 4D: CP has 2x more actions than BP (16 vs 8)")
    print("BP's advantage grows with dimensionality.")
    print(f"{'='*70}")
